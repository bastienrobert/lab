...
export default class DirtCube {
  ...
  createTexture() {
    ...
    // Setting a default black texture waiting the image
    this.gl.texImage2D(
      this.gl.TEXTURE_2D,
      0,
      this.gl.RGB,
    ...
    // On image load, set the image texture
  ...
  createBox() {
    const { vertices, indices, textures } = CubeGeometry
    ...
    // Create texture buffer, bind it and add datas
    ...
    // Get texture coordinates position attribute location in the array of attributes
    const textureLocation = this.gl.getAttribLocation(this.program, 'texture')

    this.gl.vertexAttribPointer(
      textureLocation, // Attribute position
